      ...
      <nav className="tp-main-menu-content">
        ...
        <ul className="dropdown-opened">
          {menu_data.map((menu) => {
            ...
            return (
              <li key={menu.id} className={`${has_dropdown ? 'has-dropdown' : ''} ${menu.home_dropdown || menu.pages_dropdown ? "tp-static" : ""} ${navTitle === menu.title ? "dropdown-opened expanded" : ""}`}>
                <Link href={menu.link} className={`${menu.home_dropdown || menu.pages_dropdown ? "tp-static" : ""}`}>
                  {menu.title} {has_dropdown && <button onClick={() => openMobileMenu(menu.title)} className={`dropdown-toggle-btn ${navTitle === menu.title ? "dropdown-opened" : ""}`}></button>}
                </Link>
